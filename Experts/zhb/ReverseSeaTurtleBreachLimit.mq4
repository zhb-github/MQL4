...
//+------------------------------------------------------------------+
//  反向 海龟交易系统的实现，包括短时突破，长时间保护，定时退出，使用限价停止单
// 短期突破 上次突破盈利，下次就略过，
// 长期突破，不设置保护性
// 定时退出，把交易让给时间
	...
	public:
		...
		void execute() {
			...
			if(RUN_MODE == SellOnlyMode || RUN_MODE == NormalMode){
				...
				Trade:: sellLimit(symbol,availableTp(highPrice,OP_BUY),calcLots(),magicNumber,COMMENT);
			...
			if(RUN_MODE == BuyOnlyMode || RUN_MODE == NormalMode){
				...
				Trade:: buyLimit(symbol,availableTp(lowPrice,OP_SELL),calcLots(),magicNumber,COMMENT);
		...
		void execute() {
			// 是否需要修改订单
			 if(ArraySize(buyStopOrders) >0){
                ...
                highPrice = NormalizeDouble(highPrice,MarketInfo(symbol,MODE_DIGITS));
                if(needChangePrice(highPrice,sellStopOrder.openPrice)){
                    Trade::modifyStop(sellStopOrder.ticket,availableTp(highPrice,OP_BUY),0,0);
                    printf("BlankHandle  buyStopOrders ,orderStatus: %i, highPrice: %g, availableTp: %g, buyOrderPrice: %g",orderStatus,highPrice,availableTp(highPrice,OP_BUY),sellStopOrder.openPrice);
                }
            }
            ...
            if(ArraySize(sellStopOrders) >0){
                ...
                lowPrice = NormalizeDouble(lowPrice,MarketInfo(symbol,MODE_DIGITS));
                if(needChangePrice(lowPrice,buyStopOrder.openPrice)){
                    Trade::modifyStop(buyStopOrder.ticket,availableTp(lowPrice,OP_SELL),0,0);
                    printf("BlankHandle  sellStopOrder ,orderStatus: %i, lowPrice: %g, availableTp: %g, buyStopOrderPrice: %g",orderStatus,lowPrice,availableTp(lowPrice,OP_SELL),buyStopOrder.openPrice);
                }
            }
		}
};
...
void initWork (){
    ...
    Trade:: loadOrders(buyStopOrders,symbol,OP_BUYLIMIT,magicNumber);
    Trade:: loadOrders(sellStopOrders,symbol,OP_SELLLIMIT,magicNumber);

	if(ArraySize(buyStopOrders) == 0){
        Trade:: loadOrders(buyStopOrders,symbol,OP_SELLSTOP,magicNumber);
	}
	if(ArraySize(sellStopOrders) == 0){
