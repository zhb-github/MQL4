...
//+------------------------------------------------------------------+
/*
货币对：EUR/USD 时间周期：d1 技术指标： 平均移动线 三线
开仓条件：买入条件
平仓条件：*/
...
string returnMarketInfomation() {
	...
	// 最新的均线数据
	double  newPriceMax = NormalizeDouble(iMA(symbol,intervel,maMax * KRate,0,MODE_SMA,PRICE_CLOSE,0),4);
	double  newPriceMin= NormalizeDouble(iMA(symbol,intervel,maMin * KRate,0,MODE_SMA,PRICE_CLOSE,0),4);
	double  newPriceMid= NormalizeDouble(iMA(symbol,intervel,maMid * KRate,0,MODE_SMA,PRICE_CLOSE,0),4);

	// 上一个均线数据
	double oldPriceMax = NormalizeDouble(iMA(symbol,intervel,maMax* KRate,0,MODE_SMA,PRICE_CLOSE,1),4);
	double oldPriceMin = NormalizeDouble(iMA(symbol,intervel,maMin * KRate,0,MODE_SMA,PRICE_CLOSE,1),4);
	double oldPriceMid = NormalizeDouble(iMA(symbol,intervel,maMid * KRate,0,MODE_SMA,PRICE_CLOSE,1),4);


	bool gtPriceFlag = newPriceMin > newPriceMid > newPriceMax;
	bool ltPriceFlag = newPriceMin < newPriceMid < newPriceMax;
	// 上穿 慢线
	if(newPriceMin > newPriceMax && oldPriceMin <= oldPriceMax && gtPriceFlag ){
		marketInfoResult = "buy";
	}
	// 上穿 中线
	if(newPriceMin > newPriceMid && oldPriceMin <= oldPriceMid){
		marketInfoResult = "upCrossMid";
	}
	// 下穿中线
	if(newPriceMin < newPriceMid && oldPriceMin >= oldPriceMid){
		marketInfoResult = "downCrossMid";
	}
	// 下穿
	if(newPriceMin < newPriceMax && oldPriceMin >= oldPriceMax && ltPriceFlag){
